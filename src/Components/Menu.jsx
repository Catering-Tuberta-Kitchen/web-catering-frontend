      ...
      try {
        ...
        const mappedProducts = productsArray
            ...
            .map((product) => ({
              ...
              category: product.category.name, // Tambahkan kategori ke produk
            }));
      ...
      <div className="p-6 bg-MAIN relative pt-24 pb-20">
        ...
        {showAlert && (
            <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50 z-50">
              <div className={`bg-white p-4 rounded-md shadow-lg text-center ${!user ? "max-w-sm" : "max-w-3xl"} w-72 md:w-full`}>
                <p className="text-md md:text-xl font-bold text-gray-800">{alertMessage}</p>
        ...
        {Object.entries(groupedProducts).map(([category, products]) => (
            <div key={category}>
              <h2 className="flex justify-center text-PrimFont text-3xl md:text-4xl font-lexend mt-8 mb-4">{category}</h2>
                    ...
                    >
                      <div>
                        <img
                            ...
                            alt={product.name}
                            className="h-52 w-full sm:h-48 md:h-60 object-cover rounded-md mb-4"
                        />
